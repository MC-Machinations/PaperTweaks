...
public class MiniMessageComponentRenderer extends TranslatableComponentRenderer<Locale> {
    ...
    public @NotNull Component render(@NotNull Component component, @NotNull Locale locale) {
        if (component instanceof MiniComponent miniComponent) {
            final var message = TranslationRegistry.translate(miniComponent.key(), locale);
            if (message.isEmpty()) {
                final var builder = translatable().key(miniComponent.key());
                if (miniComponent.args().isEmpty()) {
                    final var args = new HashMap<>(miniComponent.args());
            ...
            builder.append(MiniMessage.get().parse(message.get(), this.createTemplates(miniComponent, locale)));
